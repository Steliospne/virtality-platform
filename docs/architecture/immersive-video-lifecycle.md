...
- **One object per video.** Raw video lands on `immersive-videos/{id}.{ext}`; a bundle keeps the filename Unity generated, `immersive-videos/{unity filename}.bundle`, because the **Addressables Catalog** resolves it by that name (ADR 0012). A replace is a multipart upload onto the new key (for raw video that is the same key, and S3 swaps the bytes at `CompleteMultipartUpload`; a rebuilt bundle has a new hashed name). The previous key is deleted once the new one verifies. There are no retired objects and no object sweep; versioned keys return with versioned distribution. Because the URL is stable, the Download Descriptor appends `?v={version}` so CloudFront caches each version separately (ADR 0011). A headset mid-download of the previous version sees a length mismatch and reports `checksum_mismatch`; the physio's next Download starts the new version.
...
- **The Video ID may be admin-chosen, once.** The first upload for a row may carry a `videoId` (`^[a-z0-9][a-z0-9._-]{0,63}$`, unique); the row is renamed before the multipart upload is created, so the raw-video object key, the Addressables address, the console catalog and every headset agree on it. Blank keeps the generated cuid. Once a file has verified (`version ≥ 1`) the id is locked: headsets may hold files under it.
- **The Addressables Catalog is released outside this state machine.** An admin uploads the `catalog_<ts>` pair from the same page; it has no row and no state. The catalog is written first and the `.hash` last with `Cache-Control: no-cache`, so the moment the `.hash` lands the release is live for every headset that polls it. Every bundle the catalog names must already be `Published`; the platform does not parse the catalog to check (ADR 0012). Older pairs stay on the CDN and are inert.
...
| ---------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Physio closes the console tab                                          | Nothing changes. HTTP download continues. Progress events go nowhere and no console writes the mirror, so it stays at the last event seen (ADR 0013).                                                                                        | On next connect: `videoLibraryStateRequest` → current state (possibly already `ready`). Any other console viewing the headset offline already sees `ready` from the Library Mirror.                                                                                           |
...
| Headset goes to sleep (app suspended)                                  | On suspend: abort the stream, flush `.part` and manifest. On wake: reconcile, resume every `downloading` entry from its offset, rejoin the room and send `videoLibraryState` on `roomComplete`. Same as a wifi drop from the console's side. | Headset offline for the nap; progress continues on wake. Library Mirror shows `downloading` in between.                                                                                                                                                                       |
| Physio presses Pause                                                   | `videoDownloadPause` → abort within 1 s, keep `.part`, manifest `paused`, reply `videoDownloadPaused {bytesDownloaded}`. Not resumed on launch/wake.                                                                                         | Row shows "Paused at 43 % · 2.1 GB left" with **Resume** and **Cancel**. Resume sends the same `videoDownloadStart [videoId]`; the headset re-fetches the descriptor and continues from the offset. Any other console (or the offline Library Mirror view) sees `paused` too. |
...
sequenceDiagram
    ...
    Console->>API: deviceVideo.reportLibraryState (Library Mirror)
...
The Library Mirror is a write-behind cache of the headset library lifecycle, keyed by **Headset Identity**, written by the console from the headset's socket events. It exists for exactly one reason: to show a physio what is on a headset that is currently **off**, plus which downloads have been requested and not yet acknowledged. See [ADR 0009](../adr/0009-headset-owned-video-library-mirror.md) (source of truth, keying) and [ADR 0013](../adr/0013-console-written-library-mirror-with-requested-intent.md) (writer, `requested`).

```mermaid
flowchart LR
    VR -- socket: videoLibraryState / Ack / Progress / Paused / Complete / Failed --> Console[Console, headset online]
    Console -- deviceVideo.reportLibraryState / applyEvent / requestDownload / remove --> API
...
- **The console is the writer; the headset never calls the API for this.** `videoLibraryState` → `reportLibraryState` (full replace, `freeBytes`, `reportedAt = now()`); `videoDownloadAck`/`Paused`/`Complete`/`Failed` → `applyEvent` (one row); progress → `applyEvent` at most every 10 s per video. Writes are fire-and-forget and scoped to a headset on one of the caller's non-deleted `Device` rows.
- **A Download Request is written first as `requested`.** Clicking Download writes `requested` before the headset answers. A full replace keeps `requested` rows the headset did not mention; the row moves on when the headset reports that video (usually `videoDownloadAck` → `downloading`), and is removed when the physio cancels (`remove`) without waiting for the headset.
- **Snapshots plus patches.** `videoLibraryState` is the whole library and resets everything except unmentioned `requested` rows; event patches never add or remove a row except a `cancelled` failure, which removes it. Absent is "no row".
...
- **Accepted staleness.** If no console is in the room when a download finishes, the mirror keeps the last event a console saw until the next console joins and receives `videoLibraryState`. The offline view labels rows "as of `reportedAt`" for this reason.
- **The Library Mirror is barely read while the headset is online.** If the room is complete, the console renders from live Library State, overlaying only `requested` rows the headset has not reported so the physio can withdraw them. This avoids ever showing a stale `Ready` for a video the headset just deleted, and covers a report the headset failed to deliver.
