export const commonPartsEnum = {
  neck: 'neck',
  'feet-right': 'Feet Right',
  'feet-left': 'Feet Left',
  'inner-thigh-right': 'Inner Thigh Right',
  'inner-thigh-left': 'Inner Thigh Left',
  'wrist-extensors-right': 'Wrist Extensors Right',
  'wrist-extensors-left': 'Wrist Extensors Left',
  'wrist-flexors-right': 'Wrist Flexors Right',
  'wrist-flexors-left': 'Wrist Flexors Left',
  'hands-right': 'Hands Right',
  'hands-left': 'Hands Left',
  'soleus-right': 'Soleus Right',
  'soleus-left': 'Soleus Left',
  'gastrocnemius-right': 'Gastrocnemius Right',
  'gastrocnemius-left': 'Gastrocnemius Left',
  'lateral-deltoid-right': 'Lateral Deltoid Right',
  'lateral-deltoid-left': 'Lateral Deltoid Left',
} as const

export const bodyPartsFrontEnum = {
  ...commonPartsEnum,
  'upper-trapezius-right': 'Upper Trapezius Right',
  'upper-trapezius-left': 'Upper Trapezius Left',
  'anterior-deltoid-right': 'Anterior Deltoid Right',
  'anterior-deltoid-left': 'Anterior Deltoid Left',
  'upper-pectoralis-right': 'Upper Pectoralis Right',
  'upper-pectoralis-left': 'Upper Pectoralis Left',
  'mid-lower-pectoralis-right': 'Mid Lower Pectoralis Right',
  'mid-lower-pectoralis-left': 'Mid Lower Pectoralis Left',
  'long-head-bicep-right': 'Long Head Bicep Right',
  'long-head-bicep-left': 'Long Head Bicep Left',
  'short-head-bicep-right': 'Short Head Bicep Right',
  'short-head-bicep-left': 'Short Head Bicep Left',
  ...
  groin: 'Groin',
  'inner-quadricep-right': 'Inner Quadricep Right',
  'inner-quadricep-left': 'Inner Quadricep Left',
  'rectus-femoris-right': 'Rectus Femoris Right',
  'rectus-femoris-left': 'Rectus Femoris Left',
  'outer-quadricep-right': 'Outer Quadricep Right',
  'outer-quadricep-left': 'Outer Quadricep Left',
  'tibialis-right': 'Tibialis Right',
  'tibialis-left': 'Tibialis Left',
} as const

export const bodyPartsBackEnum = {
  ...commonPartsEnum,
  'upper-trapezius': 'Upper Trapezius',
  'medial-hamstrings-right': 'Medial Hamstrings Right',
  'medial-hamstrings-left': 'Medial Hamstrings Left',
  'lateral-hamstrings-right': 'Lateral Hamstrings Right',
  'lateral-hamstrings-left': 'Lateral Hamstrings Left',
  ...
  'lats-right': 'Lats Right',
  'lats-left': 'Lats Left',
  'medial-head-triceps-right': 'Medial Head Triceps Right',
  'medial-head-triceps-left': 'Medial Head Triceps Left',
  'long-head-triceps-right': 'Long Head Triceps Right',
  'long-head-triceps-left': 'Long Head Triceps Left',
  'lateral-head-triceps-right': 'Lateral Head Triceps Right',
  'lateral-head-triceps-left': 'Lateral Head Triceps Left',
  'posterior-deltoid-right': 'Posterior Deltoid Right',
  'posterior-deltoid-left': 'Posterior Deltoid Left',
  'lower-trapezius': 'Lower Trapezius',
  'traps-middle': 'Traps Middle',
  lowerback: 'Lower Back',
} as const

export type BodyPartsFrontKey = keyof typeof bodyPartsFrontEnum
export type BodyPartsBackKey = keyof typeof bodyPartsBackEnum
export type HumanState = {
  [bodyPart in BodyPartsFrontKey | BodyPartsBackKey]: { isActive: boolean }
}
...
export const bodyPartsFront = [
  ...commonParts,
  'upper-trapezius-right',
  'upper-trapezius-left',
  'anterior-deltoid-right',
  'anterior-deltoid-left',
  'upper-pectoralis-right',
  'upper-pectoralis-left',
  'mid-lower-pectoralis-right',
  'mid-lower-pectoralis-left',
  'long-head-bicep-right',
  'long-head-bicep-left',
  'short-head-bicep-right',
  'short-head-bicep-left',
  'obliques-right',
  'obliques-left',
  'upper-abdominals',
  'lower-abdominals',
  'groin',
  'inner-quadricep-right',
  'inner-quadricep-left',
  'rectus-femoris-right',
  'rectus-femoris-left',
  'outer-quadricep-right',
  'outer-quadricep-left',
  'tibialis-right',
  'tibialis-left',
]

export const bodyPartsBack = [
  ...commonParts,
  'lowerback',
  'upper-trapezius',
  'medial-hamstrings-right',
  'medial-hamstrings-left',
  'lateral-hamstrings-right',
  'lateral-hamstrings-left',
  ...
  'lats-right',
  'lats-left',
  'medial-head-triceps-right',
  'medial-head-triceps-left',
  'long-head-triceps-right',
  'long-head-triceps-left',
  'lateral-head-triceps-right',
  'lateral-head-triceps-left',
...
export const frontDefaultState = bodyPartsFront.reduce((current, next) => {
  return { ...current, [next]: { isActive: false } }
}, {} as HumanState)

export const backDefaultState = bodyPartsBack.reduce((current, next) => {
  return { ...current, [next]: { isActive: false } }
}, {} as HumanState)

/** Merges partial/raw value with default so every part key exists with valid { isActive }. */
export function normalizeFrontState(
  partial: HumanState | null | undefined,
): HumanState {
  const merged = { ...frontDefaultState }
  if (partial && typeof partial === 'object') {
    for (const key of bodyPartsFront as BodyPartsFrontKey[]) {
      const entry = partial[key]
      if (entry && typeof entry.isActive === 'boolean') {
        merged[key] = entry
      }
    }
  }
...
/** Merges partial/raw value with default so every part key exists with valid { isActive }. */
export function normalizeBackState(
  partial: HumanState | null | undefined,
): HumanState {
  const merged = { ...backDefaultState }
  if (partial && typeof partial === 'object') {
    for (const key of bodyPartsBack as BodyPartsBackKey[]) {
      const entry = partial[key]
      if (entry && typeof entry.isActive === 'boolean') {
        merged[key] = entry
      }
...
export const bodyFrontComponents = [
  {
    neck: (
      <path
...
export const bodyBackComponents = [
  {
    neck: (
      <path
