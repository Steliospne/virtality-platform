...
| `videoDownloadCancel [videoId]`                | Abort if running, dequeue if queued, delete `.part` whatever the state, manifest entry removed.                                                                                                                                                                                          | `videoDownloadCancelAck [videoId]` then `videoDownloadFailed {cancelled}`                                                                           |
| `videoDelete [videoId]`                        | If this video is playing, stop playback first (**open**: or refuse). Delete the final file, `.part`, manifest entry.                                                                                                                                                                     | `videoDeleteAck [videoId]`, then `videoLibraryState`                                                                                                |
| `videoPlay [videoId]`                          | Open the player, start from 0.                                                                                                                                                                                                                                                           | `videoPlayAck [videoId]`, then `videoPlaybackProgress` 1/s                                                                                          |
...
| `videoStop [videoId]`                          | Stop, return to idle scene.                                                                                                                                                                                                                                                              | `videoStopAck [videoId]`, then `videoEnded` (no payload)                                                                                            |
...
6. **Finish:** on EOF compare the `.part` size with `sizeBytes`. Match → rename `.part` → `{videoId}.{ext}` atomically, manifest `ready`, `videoDownloadComplete [videoId]` (the console then sends `videoLibraryStateRequest` to read the version). Mismatch → delete `.part`, `videoDownloadFailed {checksum_mismatch}`. There is no content hash to compare: integrity is verified when the file enters the platform (S3 part checksums), and the byte count is the headset's only check.
...
- `videoDownloadCancel`: same abort, then delete the `.part` and the entry. Reply `videoDownloadCancelAck [videoId]` and `videoDownloadFailed {cancelled}`.
